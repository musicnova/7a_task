...
def calc_02(src, dst, from_dt, to_dt):
...
if __name__ == '__main__':
    spark = SparkSession.builder.appName("calc_02_task").enableHiveSupport().getOrCreate()
    spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")
    hivecontext = HiveContext(spark.sparkContext)
    hivecontext.setConf("hive.exec.dynamic.partition", "true")
    hivecontext.setConf("hive.exec.dynamic.partition.mode", "nonstrict")
    spark.sparkContext.setCheckpointDir("hdfs:///user/airflow/psg/calc_02_task")
    ...
    calc_02(prod_src(), prod_dst(), opts["from_dt"], opts["to_dt"])
